...
export default function BusinessDetailPage() {
  ...
  const businessId = typeof params.id === "string" ? params.id : params.id?.[0];
  ...
  function fillForm(nextBusiness: Business) {
  ...
  async function loadBusiness() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Failed to load business.");
      }
      ...
      <div className="mx-auto max-w-6xl">
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
                Admin setup page for business profile, assigned users, WhatsApp
                number configuration, and readiness checks. Private operational
                data is shown only as counts.
              </p>
            </div>

            <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 md:min-w-[220px]">
              <p className="text-sm text-emerald-100/80">Setup readiness</p>
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <MessageCircle className="mb-4 h-5 w-5 text-emerald-400" />
            <p className="text-3xl font-black">
              {formatNumber(counts.conversations)}
            </p>
            <p className="mt-1 text-xs text-slate-500">Conversation count</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <Users className="mb-4 h-5 w-5 text-emerald-400" />
            <p className="text-3xl font-black">
              {formatNumber(counts.customers)}
            </p>
            <p className="mt-1 text-xs text-slate-500">Customer count</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <Database className="mb-4 h-5 w-5 text-emerald-400" />
            <p className="text-3xl font-black">
              {formatNumber(counts.knowledge_entries)}
            </p>
            <p className="mt-1 text-xs text-slate-500">Knowledge entries</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <Smartphone className="mb-4 h-5 w-5 text-emerald-400" />
            <p className="text-3xl font-black">
              {formatNumber(phoneNumbers.length)}
            </p>
            <p className="mt-1 text-xs text-slate-500">WhatsApp numbers</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <ShieldCheck className="mb-4 h-5 w-5 text-emerald-400" />
            <p className="text-3xl font-black">
              {counts.ai_settings > 0 ? "Yes" : "No"}
            </p>
            <p className="mt-1 text-xs text-slate-500">AI settings</p>
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  rows={5}
                  placeholder="Business description, notes, setup status..."
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="mt-5 inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSaving ? (
                <>
