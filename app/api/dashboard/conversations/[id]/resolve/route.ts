...
export async function PATCH(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!canAccess) {
      return NextResponse.json(
        { error: "You do not have permission to resolve this conversation." },
        { status: 403 }
      );
    }
    ...
    await saveSystemMessage(
      supabase,
      typedConversation,
      `Conversation marked resolved by ${profile.full_name || profile.email || "staff"}. AI remains paused.`,
      profile
    );
